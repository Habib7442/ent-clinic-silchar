      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8 lg:auto-rows-[240px]">
          {homepageItems.map((item) => (
            <div 
              key={item.id} 
              className={`gallery-item relative overflow-hidden rounded-sm group ${item.spanHomepage} aspect-[16/10] lg:aspect-auto lg:h-full`}
            >
              <img
                src={item.src}
              ...
              />
              {/* Overlay */}
              <div className="absolute inset-0 bg-gradient-to-t from-[#0A1A12]/90 via-[#0A1A12]/20 to-transparent opacity-75 group-hover:opacity-90 transition-opacity duration-500" />
